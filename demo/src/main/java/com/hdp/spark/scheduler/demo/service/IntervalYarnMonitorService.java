...
public final class IntervalYarnMonitorService {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(IntervalYarnMonitorService.class);

    private final TaskRegistry registry;
    private final HadoopYarnApplicationClient yarnClient;
    private final SchedulerProperties properties;

    public IntervalYarnMonitorService(TaskRegistry registry, HadoopYarnApplicationClient yarnClient,
    ...
    /**
     * 启动 YARN 状态监控进程。需求里是每半小时查一把 YARN，所以这里 initialDelay 也设置为半小时。
     */
    ...
    public void safeRefreshOnce() {
        try {
            ...
            LOGGER.error("Interval YARN monitor failed", e);
        }
    }
    ...
    public void refreshOnce() {
        for (IntervalTriggerTaskInstance task : registry.intervalTasks()) {
            try {
                ...
                LOGGER.warn("Failed to query YARN status for interval task {}", task.taskName(), e);
            }
        }
    }
}