...
### 11.12 MULTI-POLLING 保留并实现 scheduler.task-timeout

状态：已达成初步共识
适用模式：`MULTI-POLLING`

想法：
`scheduler.task-timeout` 在 `MULTI-POLLING` 下也应保留并真正生效，表示单个轮询 task run 的最大执行时间；`app.max-running-duration` 继续表示整个常驻 Spark 应用的最长运行时间。

判断：
这个方向合理，并且应与 `MULTI-ONCE` 保持一致。`MULTI-POLLING` 中同一个任务会按轮次重复提交，如果某一轮 task run 卡住，当前代码会因为 `runningTaskIds` 仍包含该任务而跳过后续轮次。因此单任务级超时对轮询模式更重要，它可以避免某个任务长期占住运行状态，导致后续轮次一直跳过。

建议目标行为：

- `scheduler.task-timeout` 可选；未配置时表示不启用单任务级超时。
- 配置后，超时时间从 task run 真正开始执行时计算，不包含在线程池队列中等待的时间。
- 某一轮 task run 超时后，该 run 标记为 `FAILED` 或 `CANCELLED`，具体状态后续落地时统一定义。
- 超时后必须释放该 taskId 的运行标记，避免后续 polling round 一直跳过。
- 如果 `scheduler.fail-fast=true`，单任务超时应触发整个 `MULTI-POLLING` 提前收敛。
- 如果 `scheduler.fail-fast=false`，只结束该超时 run，后续轮次仍可继续提交同一个 taskId。
...
- 保留 `SchedulerConfig.taskTimeout` 和解析逻辑。
- 在 `TaskManager` 中实现单任务级超时控制，并同时覆盖 `MULTI-ONCE` 和 `MULTI-POLLING`。
- 不能只依赖 `Future.cancel(true)`，因为 Spark SQL 已提交的 job 不一定会因为线程中断而可靠停止。
- `TaskRunner` 执行每个 task run 前应设置独立 Spark job group，例如基于 `taskId + runId` 生成 groupId。
- 任务超时时，`TaskManager` 应调用 `sparkContext.cancelJobGroup(groupId)` 取消该 task run 对应的 Spark job。
- `TaskRunner` 结束后清理 job group/local properties，避免影响线程池复用后的下一个任务。
- 示例和文档继续保留 `scheduler.task-timeout`，并说明它是单个 task run 超时，不是应用级超时。
...
- `app.runtime-mode` 保留但不必填；不填默认 `BATCH`，显式配置时只能是 `BATCH`。
- `spark` 节只允许 `config`。
- `scheduler.task-timeout` 保留，并作为单个 polling task run 超时生效。
